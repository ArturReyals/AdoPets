  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        <button onClick={() => mudarMes(-1)} style={{ borderRadius: '50%', width: '36px', height: '36px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
        ...
        {dias.map(d => {
          ...
          return (
            <div key={d}
              onClick={() => !desativado && onSelect(data)}
              style={{
                textAlign: 'center', borderRadius: '10px', padding: '10px 4px', fontSize: '14px',
                cursor: desativado ? 'default' : 'pointer', fontWeight: isSel ? 700 : 500,
                background: isSel ? ROSA : isHoje ? AMARELO : 'transparent',
                color: isSel ? 'white' : isHoje ? ROSA : desativado ? '#ccc' : '#333',
                border: isSel ? `2px solid ${ROSA}` : '2px solid transparent',
                transition: 'all 0.2s',
              }}
            >{d}</div>
          );
          ...
          <React.Fragment key={n}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, maxWidth: '130px' }}>
              <div style={{
                ...
                border: `2px solid ${done || active ? ROSA : '#ddd'}`,
                background: done || active ? ROSA : 'white',
                color: done || active ? 'white' : '#aaa',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 700, fontSize: '16px', transition: 'all 0.4s',
              }}>
            ...
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: '2px', background: done ? ROSA : '#ddd', marginBottom: '20px', transition: 'background 0.4s' }} />
            )}
          </React.Fragment>
        );
      })}
...
export default function Agendamento() {
  ...
  function irPara(n) {
    ...
    if (n === 4) {
      ...
      setProtocolo(prot);
      setTela(4);
      // simula progressão dos estágios
      ...
      <section style={{ backgroundColor: AMARELO, padding: '70px 0 90px', textAlign: 'center' }}>
        <div className="container">
          <h1 style={{ color: ROSA, fontWeight: 800, fontSize: '2.8rem' }}>
            <i className="bi bi-calendar-heart me-2"></i>Agendar Visitação
          </h1>
          <p style={{ color: ROSA, fontSize: '1.2rem', marginTop: '12px' }}>
            Venha conhecer nossos pets pessoalmente! Escolha o melhor dia e horário para sua visita.
          </p>
        </div>
      </section>
          ...
          {tela === 2 && (
            <>
                ...
                <div style={{ marginTop: '24px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                    {HORARIOS.map(h => {
                      ...
                      return (
                        <div key={h}
                          onClick={() => !ocupado && setHorarioSel(h)}
                          style={{
                            border: `2px solid ${selecionado ? ROSA : ocupado ? '#f0f0f0' : '#ddd'}`,
                            borderRadius: '12px', padding: '12px 8px', textAlign: 'center',
                            background: selecionado ? ROSA : ocupado ? '#fafafa' : 'white',
                            color: selecionado ? 'white' : ocupado ? '#ccc' : '#333',
              ...
              <div className="d-flex justify-content-between mt-4">
                ...
                <button onClick={() => irPara(3)} style={{ ...btnRosa, opacity: !diaSel || !horarioSel ? 0.5 : 1 }} disabled={!diaSel || !horarioSel}>
          ...
          {tela === 3 && (
            <>
              ...
              <div className="d-flex justify-content-between">
                ...
                <button onClick={() => irPara(4)} style={btnRosa}><i className="bi bi-check-circle me-1"></i> Confirmar Visita</button>
              </div>
            </>
          )}
            ...
            <>
              ...
              <div style={{ marginTop: '24px' }}>
                {[
                  ...
                  const isFirst = i === 0;
                  const prevDone = i === 0 || stages[Object.keys(stages)[i - 1]];
                  const current = prevDone && !done && !isFirst;
                  return (
                    <div key={stage.key} style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', padding: '14px 0', borderBottom: i < 4 ? '1px solid #f0f0f0' : 'none' }}>
                      <div style={{
                        ...
                        background: done ? ROSA : current ? AMARELO : '#f0f0f0',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: done ? 'white' : current ? ROSA : '#ccc',
              ...
              <div className="text-center mt-4 d-flex justify-content-center gap-3 flex-wrap">
                ...
                <Link to="/pets" className="btn rounded-pill px-4" style={{ background: ROSA, color: 'white', fontWeight: 700 }}>Ver Pets</Link>
              </div>
            </>
          )}
        </div>
